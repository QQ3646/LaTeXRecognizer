...
def inkml2truth(input_path):
    tree = ET.parse(input_path)
    root = list(tree.getroot())
    for elem in root:
        if elem.attrib.get('type') == "truth":
            return elem.text
    return None


def run(input_inkml, output_png_path):
    try:
        inkml2img(input_inkml, output_png_path, color='#284054')
        print(f"{input_inkml} is created ({output_png_path})!")
        text = inkml2truth(input_inkml)
        if text is not None:
            text = text.replace('amp;', '')
            return text
    except:
        print(f"{input_inkml} is skipped!")
        return None
